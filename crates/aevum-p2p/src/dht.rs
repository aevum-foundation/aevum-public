...
const K: usize = 8; // max nodes per bucket (like in BitTorrent)

/// Kademlia Distributed Hash Table
pub struct Dht {
    /// 256 buckets by XOR distance from our ID
    buckets: [Vec<DhtNode>; K_BUCKETS],
    /// Our ID (first 32 bytes = hash of public key)
    our_id: [u8; 32],
}
...
impl Dht {
    ...
    /// Which bucket a node falls into (by highest bit of XOR distance)
    /// If distance == 0 (our own ID) → bucket 0, but add_or_update filters it out
    ...
    pub fn add_or_update(&mut self, node_id: [u8; 32], addr: SocketAddr, now: u64) {
        ...
        if bucket.len() < K {
            ...
            // Kademlia: ping oldest, replace if no response
            // Simplified: remove the oldest
            bucket.sort_by_key(|n| n.last_seen);
            bucket[0] = node;
        }
    }

    /// Find K closest LIVING nodes to target_id
    pub fn find_closest(&self, target_id: &[u8; 32], count: usize, now: u64, timeout_secs: u64) -> Vec<DhtNode> {
        ...
        // Collect from buckets from nearest to farthest
    ...
    /// Random living nodes (for replication) — proper shuffle
    ...
    /// Get random IDs for refreshing each bucket
...
/// Convert [u8; 32] to a sortable key
...
mod tests {
    ...
    fn random_nodes_is_random() {
        ...
        // May coincide, but probability is low with 20 nodes
        let same = r1.iter().zip(r2.iter()).filter(|(a, b)| a.node_id == b.node_id).count();
        assert!(same < 4); // not all 4 are the same
    }
    ...
    fn cleanup_removes_dead() {
        ...
        assert_eq!(dht.total_nodes(), 1); // only [2u8; 32] is alive
    }
