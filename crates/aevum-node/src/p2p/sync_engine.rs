...
impl SyncEngine {
    ...
    /// Обновить высоту пира (вызывается при получении нового Status)
    pub fn update_peer_height(&mut self, peer_height: u64) {
        if peer_height > self.peer_height {
            tracing::info!("[Sync] Peer height updated: {} -> {}", self.peer_height, peer_height);
            self.peer_height = peer_height;
        }
    }
    ...
    fn request_headers(&mut self) {
        ...
        if let Ok(data) = bincode::serialize(&req) {
            ...
            tracing::info!("[Sync] Requested headers {}-{}", our_height + 1, self.peer_height);
        }
    }
        ...
        match st.load_block(our_height) {
            Ok(Some(last_block)) => {
                if self.headers[0].prev_hash != last_block.block_hash.0 {
                    tracing::warn!("[Sync] First header prev_hash mismatch");
                    return false;
                }
            }
            _ => { tracing::warn!("[Sync] Block {} not found", our_height); return false; }
        }
    ...
    fn request_blocks(&mut self) {
        ...
        if let Ok(data) = bincode::serialize(&req) {
            self.peers.send_to(&self.peer_id, data);
            tracing::info!("[Sync] Requested blocks {}-{} (req={})", from, to, request_id);
        }
    }
    ...
    pub fn on_blocks(&mut self, blocks: Vec<(u64, Vec<u8>)>, request_id: u64) {
        ...
        if our_height >= self.peer_height {
            self.state = SyncState::Synced;
            tracing::info!("[Sync] Complete! Height now {}", our_height);
            return;
        }

        // Запрашиваем следующую партию
        let from = our_height + 1;
        let to = (from + MAX_BLOCKS_PER_REQUEST - 1).min(self.peer_height);
        let request_id = rand::random();
        ...
        if let Ok(data) = bincode::serialize(&req) {
            self.peers.send_to(&self.peer_id, data);
            tracing::info!("[Sync] Requested next batch {}-{} (req={})", from, to, request_id);
        }
    }
    ...
    fn retry_or_fail(&mut self, reason: &str) {
        ...
        tracing::warn!("[Sync] {} — retry {}/{}", reason, self.retry_count, MAX_RETRIES);
        self.request_headers();
    }

    /// Проверить таймауты — вызывается из AtpConnection
    ...
    /// Обработать сообщение — возвращает true если сообщение обработано
    pub fn handle_message(&mut self, msg: &AtpMessage) -> bool {
        // Обновляем peer_height если пришёл новый Status
